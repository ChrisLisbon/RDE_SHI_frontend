...
async function pushNewHydroObsCSV(file)
    {	console.log(file)
    	var formData = new FormData();
    	formData.append('csvFile', file, file.name);
      	let response = await fetch('http://localhost:4444/new_hydro_obs_csv_push', {
...
async function pushNewHydroObsXlsx(file)
    {	console.log(file)
    	var formData = new FormData();
    	formData.append('xlsxFile', file, file.name);
      	let response = await fetch('http://localhost:4444/new_hydro_obs_xlsx_push', {
        method: 'POST', // *GET, POST, PUT, DELETE, etc.
        mode: 'cors', // no-cors, cors, *same-origin
        cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
        credentials: 'same-origin', // include, *same-origin, omit
...
export default function AdminHydroObsChoise(props){
	...
	const pushCSVFileAndLog=(file)=>{
		pushNewHydroObsCSV(file).then(data=>{setLogInfo(data)})
		console.log(state)
	}
	const pushXlsxFileAndLog=(file)=>{
		pushNewHydroObsXlsx(file).then(data=>{setLogInfo(data)})
		console.log(state)
	}
